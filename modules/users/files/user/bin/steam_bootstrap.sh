...
download() {
    ...
    if [ ! -f "$TMPDIR/$filename" ]; then
        wget -c "$url" -O "$TMPDIR/$filename.part"
        mv "$TMPDIR/$filename.part" "$TMPDIR/$filename"
    fi
}
...
package() {
    ...
    mkdir -p "$target"
    ar p "$TMPDIR/$(basename "$url")" data.tar.gz | tar xz -C "$target"
}

set -e

# Set installation directory from arguments
if [ -z "$1" ]; then
  echo Specify installation directory as script parameter! >&2
  exit 1
fi

INSTALLDIR="$1"

# Set up tmp directory for installation
TMPDIR=$(mktemp -d)

package http://media.steampowered.com/client/installer/steam.deb "${TMPDIR}/tree"
...
LAUNCHSTEAMBOOTSTRAPFILE="${TMPDIR}/tree/usr/lib/steam/bootstraplinux_${LAUNCHSTEAMPLATFORM}.tar.xz"
#STEAM_DATA_HOME=${XDG_DATA_HOME:-$HOME/.local/share}
STEAM_DATA_HOME="$INSTALLDIR"
...
sed "s!/usr/bin/steam!${STEAMDIR}/steam_on_debian.sh!" $TMPDIR/tree/usr/share/applications/steam.desktop > "${STEAM_DATA_HOME}/applications/steam.desktop"
cp -R $TMPDIR/tree/usr/share/icons "${STEAM_DATA_HOME}"
